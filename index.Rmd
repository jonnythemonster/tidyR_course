...
knitr::opts_chunk$set(message=TRUE,warning=TRUE, cache=TRUE)
```

Sommersemester 2023 | Stand: `r Sys.Date()`

# Einführung
In diesem Kurs geht es um die Programmiersprache R. Doch was ist das eigebntlich?
...
Weitere Details stehen in in `?select`. Mit regular expressions, kann man auch matches() verwenden, um Variablen auszuwählen, die einem bestimmten Muster entsprechen.

Sie können Variablen während der Auswahl `select()` umbenennen, indem du `=` verwendest. Der neue Name erscheint auf der linken Seite von `=`, und die alte Variable erscheint auf der rechten Seite:
```{r}
flights  %>%  
