...
Wenn das Wetter für alle Flughäfen in den USA enthalten wäre, würde es das Wetter für den Zielort jedes Fluges liefern. Die Spalten des Wetterdatenrahmens (Jahr, Monat, Tag, Stunde, Herkunft) sind ein Fremdschlüssel für die Spalten des Flugdatenrahmens (Jahr, Monat, Tag, Stunde, Ziel). Dies würde Informationen über das Wetter am Zielflughafen zum Zeitpunkt des Abflugs liefern, es sei denn, die Ankunftszeit wurde berechnet.
...
Wir wissen, dass einige Tage im Jahr besonders sind und weniger Menschen als üblich fliegen (z.B. Heiligabend und Weihnachtstag). 
   Wie könntest du diese Daten als Datenrahmen darstellen? 
   Was wäre der Primärschlüssel? 
   Wie würde er sich mit den vorhandenen Datenrahmen verbinden?
...
# Übung 6
## Aufgabe 1
1a. Angenommen, du hast zwei Datenrahmen `df1` und `df2` mit einer gemeinsamen Spalte "ID". Führe einen `left_join()` durch und speichere das Ergebnis im Datenrahmen `merged_df`. Gib anschließend den resultierenden Datenrahmen aus.
...
1b. Gegeben sind zwei Datenrahmen `df3` und `df4` mit einer gemeinsamen Spalte "ID". Führe einen `inner_join()` durch und speichere das Ergebnis im Datenrahmen `merged_inner`. Gib anschließend den resultierenden Datenrahmen aus.
...
1c. Angenommen, du hast zwei Datenrahmen `df5` und `df6` mit einer gemeinsamen Spalte "ID". Führe einen `right_join()` durch und speichere das Ergebnis im Datenrahmen `merged_right`. Gib anschließend den resultierenden Datenrahmen aus.
...
1d. Gegeben sind zwei Datenrahmen `df7` und `df8` mit einer gemeinsamen Spalte "ID". Führe einen `full_join()` durch und speichere das Ergebnis im Datenrahmen `merged_full`. Gib anschließend den resultierenden Datenrahmen aus.
...
1e. Angenommen, du hast zwei Datenrahmen `df9` und `df10` mit einer gemeinsamen Spalte "ID". Führe einen `semi_join()` durch und speichere das Ergebnis im Datenrahmen `merged_semi`. Gib anschließend den resultierenden Datenrahmen aus.
...
1f. Gegeben sind zwei Datenrahmen `df11` und `df12` mit einer gemeinsamen Spalte "ID". Führe einen `anti_join()` durch und speichere das Ergebnis im Datenrahmen `merged_anti`. Gib anschließend den resultierenden Datenrahmen aus.
